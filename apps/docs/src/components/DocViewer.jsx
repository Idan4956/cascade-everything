...
function renderMarkdown(md) {
  ...
  html = html.replace(/(<oli>.*<\/oli>(\n|$))+/g, m => `<ol style="padding-left:20px;margin:8px 0">${m.replace(/<\/?oli>/g, t => t === '<oli>' ? '<li style="margin:3px 0">' : '</li>')}</ol>`)
  html = html.replace(/\n\n/g, '</p><p style="margin:0 0 12px;line-height:1.7">')
  return '<p style="margin:0 0 12px;line-height:1.7">' + html + '</p>'
}

const EDITABLE_TYPES = new Set(['text', 'markdown'])

export default function DocViewer({ file, data, loading, error, onOpen, onSave }) {
  ...
  useEffect(() => { setEditing(false); setEditContent(''); setSaved(false) }, [file])

  useEffect(() => {
    if (data && EDITABLE_TYPES.has(data.type)) setEditContent(data.content)
  }, [data])
  ...
  const handleChange = useCallback((val) => {
    ...
    saveTimer.current = setTimeout(async () => {
      const result = await window.docsAPI.writeFile(file, val)
      if (result.ok) setSaved(true)
    }, 800)
  }, [file])

  const handleSaveNow = useCallback(async () => {
    const result = await window.docsAPI.writeFile(file, editContent)
    if (result.ok) setSaved(true)
  }, [file, editContent])
  ...
  if (!file) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
        ...
        <button onClick={onOpen} style={{ marginTop: 8, padding: '10px 22px', borderRadius: 10, background: 'var(--accent)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>
          Open document
        </button>
      </div>
    )
  ...
  if (loading) return <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: 'var(--muted)', fontSize: 13 }}>Loading…</span></div>
  if (error) return (
  ...
  // ── PDF ──────────────────────────────────────────────────────────────────────
  if (data.type === 'pdf') {
    return <iframe src={data.url} style={{ flex: 1, border: 'none', background: '#fff' }} title="PDF Viewer" />
  }
  ...
  if (data.type === 'html') {
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '32px 48px', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', color: 'var(--text)', fontSize: 15, lineHeight: 1.7, userSelect: 'text' }}
          ...
          <div style={{ display: 'flex', gap: 2, padding: '8px 16px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            {data.sheets.map((s, i) => (
              <button key={i} onClick={() => setActiveSheet(i)} style={{
                ...
                color: activeSheet === i ? 'var(--accent)' : 'var(--muted)', cursor: 'pointer',
              }}>{s.name}</button>
            ))}
          </div>
        )}
        <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto' }}>
          <style>{`.xlsx-table table{border-collapse:collapse;font-size:13px}.xlsx-table td,.xlsx-table th{border:1px solid rgba(255,255,255,0.08);padding:6px 12px;white-space:nowrap;color:var(--text)}.xlsx-table tr:nth-child(even) td{background:rgba(255,255,255,0.02)}.xlsx-table tr:hover td{background:rgba(59,130,246,0.08)}`}</style>
  ...
  // ── Editable text / markdown ─────────────────────────────────────────────────
  if (data.type === 'markdown' || data.type === 'text') {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Toolbar */}
        <div style={{ height: 36, display: 'flex', alignItems: 'center', padding: '0 14px', gap: 6, background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <ToolBtn active={!editing} onClick={() => setEditing(false)}>Preview</ToolBtn>
          <ToolBtn active={editing} onClick={handleEdit}>Edit</ToolBtn>
          {editing && (
            <>
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: 11, color: saved ? '#22c55e' : 'var(--muted)' }}>
                {saved ? 'Saved' : 'Unsaved'}
              </span>
              <button onClick={handleSaveNow} style={{
                padding: '3px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, border: 'none',
        ...
        {editing ? (
          <textarea
            ...
            style={{
              ...
              fontFamily: data.type === 'text' ? "'Cascadia Code','Fira Code',Consolas,monospace" : "'Cascadia Code','Fira Code',Consolas,monospace",
              fontSize: 14, lineHeight: 1.75, padding: '24px 28px',
              userSelect: 'text',
            }}
          />
        ) : (
          <div style={{ flex: 1, overflowY: 'auto', padding: data.type === 'markdown' ? '32px 48px' : '24px', background: 'var(--bg)' }}>
            {data.type === 'markdown' ? (
              <div style={{ maxWidth: 720, margin: '0 auto', color: 'var(--text)', fontSize: 15, userSelect: 'text' }}
                dangerouslySetInnerHTML={{ __html: renderMarkdown(editContent || data.content) }} />
            ) : (
              <pre style={{ color: 'var(--text)', fontSize: 13, lineHeight: 1.7, fontFamily: "'Cascadia Code','Fira Code',Consolas,monospace", whiteSpace: 'pre-wrap', wordBreak: 'break-word', userSelect: 'text' }}>
